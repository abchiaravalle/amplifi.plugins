 ...
 *
 * @package Amplifi\Security\Crypto
 */

declare(strict_types=1);

namespace Amplifi\Security\Crypto;
...
final class Secret_Store {
	...
	public static function encrypt( string $plaintext ): string {
		...
		if ( false === $ciphertext ) {
			throw new RuntimeException( 'amplifi.security: encryption failed' );
		}
	...
	public static function decrypt( string $encoded ): string {
		...
		if ( ! str_starts_with( $encoded, self::PREFIX ) ) {
			throw new InvalidArgumentException( 'amplifi.security: unknown ciphertext version' );
		}

		$blob = base64_decode( substr( $encoded, strlen( self::PREFIX ) ), true );
		if ( false === $blob || strlen( $blob ) < self::IV_LENGTH + self::TAG_LENGTH + 1 ) {
			throw new InvalidArgumentException( 'amplifi.security: malformed ciphertext' );
		}
		...
		if ( false === $plaintext ) {
			throw new RuntimeException( 'amplifi.security: decryption failed (auth tag mismatch)' );
		}
