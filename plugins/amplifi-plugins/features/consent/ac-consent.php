<?php
/*
 * amplifi.consent feature module (bundled inside amplifi.plugins).
 * First-party cookie consent that hard-withholds tracking scripts until the
 * visitor accepts. Managed scripts are emitted inside inert base64 <template>
 * elements (browsers never execute their contents) and only materialized
 * after consent, nothing fires on reject. Per-category toggles, accept/reject
 * toast, version-bound consent (configurable 1-365 days), a
 * [amplifi-consent-manager] shortcode, a best-effort server-side consent log,
 * GPC support, optional auto-block of unmanaged trackers, and an admin cookie
 * scanner that loads each script in an isolated admin harness frame (a real
 * execution that may contact the third party, only scan scripts you trust)
 * to detect the cookies it sets so you can categorize them.
 * Standalone dev source: plugins/ac-consent/ac-consent.php
 */
