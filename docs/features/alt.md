...
- **Decorative results write an empty string, which the scanner treats as "no alt".** `generate()` writes `''` for decorative images by design (WCAG-correct), but the enqueue query matches `meta_value IS NULL OR meta_value = ''`. The bulk *query* therefore still matches decorative images on every run — but `enqueue()` is an `INSERT IGNORE` against `UNIQUE KEY uniq_attachment (attachment_id)`, and the decorative attachment already has a `done` row, so no new job is created and **no additional API spend occurs**. The only real effect is an inflated candidate count in the pre-flight estimate. (No code path ever deletes job rows.)
